...
import { TableInitializer } from './src/database/db.initializer'
...
export default class App {
    ...
    constructor(config: Partial<AppConfig> = {}) {
        this.config = {
            ...
            host: config.host || process.env.SERVER_HOST || '127.0.0.1',
    ...
    async start(): Promise<void> {
        try {
            ...
            // const port = process.env.NODE_ENV === 'development' ? this.config.port : process.env.PORT;
            console.log(`🚀 Démarrage serveur sur ${this.config.host}...`);

            await new Promise<void>((resolve, reject) => {
                this.server = this.app.listen(this.config.host, () => {
                ...
                this.server?.on('error', (error: any) => {
                    if (error.code === 'EADDRINUSE') {
                        reject(new Error(`Port déjà utilisé`));
