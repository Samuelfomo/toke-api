...
export default class CountryModel extends BaseModel {
    ...
    protected async create(): Promise<void> {
        ...
        const lastID = await this.insertOne(this.db.tableName, {
            ...
            [this.db.is_active]: this.is_active !== undefined ? this.is_active : true,
    ...
    protected async update(): Promise<void> {
        ...
        if (this.default_currency_code !== undefined) updateData[this.db.default_currency_code] = this.default_currency_code;
        if (this.default_language_code !== undefined) updateData[this.db.default_language_code] = this.default_language_code;
        if (this.is_active !== undefined) updateData[this.db.is_active] = this.is_active;
        if (this.timezone_default !== undefined) updateData[this.db.timezone_default] = this.timezone_default;
    ...
    private async validate(): Promise<void> {
        ...
        if (!this.default_currency_code || !CountryDbStructure.validation.validateCurrencyCode(this.default_currency_code)) {
            throw new Error('Currency code is required and must be a valid ISO 4217 code (3 uppercase letters)');
        }

        // Valider le code de langue (obligatoire)
        if (!this.default_language_code || !CountryDbStructure.validation.validateLanguageCode(this.default_language_code)) {
            throw new Error('Language code is required and must be a valid ISO 639-1 code (2 lowercase letters)');
        }

        // Valider le fuseau horaire (obligatoire avec valeur par défaut)
        if (this.timezone_default && !CountryDbStructure.validation.validateTimezone(this.timezone_default)) {
        ...
        if (!this.phone_prefix || !CountryDbStructure.validation.validatePhonePrefix(this.phone_prefix)) {
            throw new Error('Phone prefix is required and must follow the format +XXX (1-5 digits after +)');
        }

        // Valider le statut actif (optionnel avec valeur par défaut)
        if (this.is_active !== undefined && !CountryDbStructure.validation.isActive(this.is_active)) {
